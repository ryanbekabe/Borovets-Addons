...
case $1 in
	--help)
		echo "Выполнение: repackimg.sh [--original] [--level <0-9>] [--avbkey <name>] [--forceelf]"
		return 1
	;;
esac
...
if [ -z "$(ls split_img/* 2>/dev/null)" -o ! -e ramdisk ]; then
	echo "Не найдено ramdisk и split_img для сборки."
	abort
	return 1
fi
...
if [ ! "$($bb mount | $bb grep " $aik/ramdisk ")" ]; then
	...
	if [ $? != "0" ]; then
		for i in 0 1 2 3 4 5 6 7; do
			...
			$bb mknod $loop b 7 $i 2>/dev/null;m
...
if [ ! -z "$(ls *-new.* 2>/dev/null)" ]; then
	echo "Перезаписываю существующие файлы!\n";
fi
rm -f *-new.*;

if [ "$original" ]; then
	echo "Собираю с оригин. ramdisk'ом...";
elif [ "$ramdiskcomp" == "empty" ]; then
	echo "Юзаю пустой ramdisk для сборки!";
	compext=.empty;
	touch ramdisk-new.cpio$compext;
else
	echo "Собираю ramdisk...\n";
	test ! "$level" -a "$ramdiskcomp" == "xz" && level=-1;
	echo "Юзаю сжатие: $ramdiskcomp$lvltxt";
...
if [ -f split_img/*-mtktype ]; then
	...
	echo "\nГенерирую заголовкиMTK...\n";
	echo "Юзаю тип ramdisk'а: $mtktype";
...
if [ "$imgtype" == "ELF" ] && [ ! "$header" -o ! "$repackelf" ]; then
	imgtype=AOSP;
	echo "\nОбнаружил формат ELF без RPM, поэтому, будет собран в формате AOSP!";
fi

echo "\nСоздаю образ...\n";
echo "Юзаю формат: $imgtype\n";
case $imgtype in
	...
	U-Boot)
		...
		;;
	*)
		echo "\nНеподдерживаемый формат"; abort; return 1
	;;
esac
if [ $? != "0" ]; then
	abort
...
if [ -f split_img/*-sigtype ]; then
	...
	echo "Подписываю...\n";
	echo "Юзаю подпись: $sigtype $avbtype$avbtxt$blobtype\n";
...
if [ -f split_img/*-lokitype ]; then
	...
	echo "Применяю патч Loki...\n";
	echo "Юзаю тип: $lokitype\n";
	$bb mv -f image-new.img unlokied-new.img;
	if [ -f aboot.img ]; then
		$bin/loki_tool patch $lokitype aboot.img unlokied-new.img image-new.img >/dev/null;
		if [ $? != "0" ]; then
			echo "Патч провалился.";
			abort
			return 1
		fi
	else
		echo "aboot.img требуется в каталоге скриптов, что-бы найти смещение патча Loki.";
		abort
		return 1
	fi
fi
...
if [ -f split_img/*-tailtype ]; then
	...
	echo "Добавляю нижний колонтитул...\n";
	echo "Юзаю тип: $tailtype\n";
	case $tailtype in
